...
" ---------------------------------------------------------------------
" ansi_esc#BufReadPost: updates ansi-escape code visualization if it was alredy
" on for the buffer{{{2
fun! ansi_esc#BufReadPost()
  let bn= bufnr("%")
  if exists("s:AnsiEsc_enabled_{bn}") && s:AnsiEsc_enabled_{bn}
   call ansi_esc#AnsiEsc(1)
  endif
endfun
