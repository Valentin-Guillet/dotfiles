...
def get_settings_path():
    settings_path = os.environ["LOCALAPPDATA"]
    return (
        Path(settings_path)
        / "Packages"
...
def main():
    ...
    new_settings["keybindings"] = [
        obj for obj in new_settings["keybindings"] if not obj.get("toggle", False)
    ]
    ...
    if len(new_settings["keybindings"]) == len(old_settings["keybindings"]):
        ...
        new_settings["keybindings"] = [
            obj for obj in custom_settings["keybindings"] if not obj.get("toggle", False)
        ]
