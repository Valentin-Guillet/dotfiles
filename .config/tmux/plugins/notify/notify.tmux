...
main() {
    local curr_tty bash_state
    curr_tty="$(tmux display-message -p '#{pane_tty}')"
    ...
    local pane_id notif_msg
    pane_id="$(tmux display-message -p '#{pane_id}')"
    # shellcheck disable=SC2016
    notif_msg="$(tmux display-message -p 'Command \`#{pane_current_command}` in window [#S:#I###P] has finished')"

    # Cancel if already running -- check the pane fg option
    if [[ "$(tmux show-options -t "$pane_id" -p window-active-style)" ]]; then
