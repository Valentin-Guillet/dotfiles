...
def lookup_symbol_curr_view(view, symbol, pt):
    ...
    file_syntax = sublime.find_syntax_for_file(view.file_name()).name

    def create_symbol_location(row, col, kind):
        return sublime.SymbolLocation(file_name, file_rel_path, row+1, col+1,
                                      file_syntax, sublime.SYMBOL_TYPE_DEFINITION, kind)
    ...
    for line_index in range(curr_index, -1, -1):
        ...
        if (line_indent < max_indent or line_index == curr_index):
            ...
            if regex_for_loop_def.match(line_text):
                ...
                if match:
                    ...
                    locations.append(create_symbol_location(row, col, sublime.KIND_VARIABLE))
                    break
        ...
        if match:
            row, col = view.rowcol(line.a + len(match.group(1)))
            locations.append(create_symbol_location(row, col, sublime.KIND_NAMESPACE))
            break
...
def lookup_symbol_everywhere(view, symbol):
    ...
    for line_index in range(len(lines)):
        ...
        if match:
            ...
            if 'meta.function-call.arguments.python' not in view.scope_name(pt):
                row, col = view.rowcol(pt)
                locations.append(create_symbol_location(row, col, sublime.KIND_AMBIGUOUS))
                break
